...
public class HandController_antoine : MonoBehaviour {
	...
	protected bool is_hand_closed () {
		...
		if ( handType == HandType.LeftHand ) return
			OVRInput.Get( OVRInput.RawButton.X );   // Check that the index finger is pressing
		else return false;

		// // Case of a right hand
		// else return
		// 	 OVRInput.Get( OVRInput.Axis1D.RawButton.X ) > 0.5; // Check that the index finger is pressing
	}
