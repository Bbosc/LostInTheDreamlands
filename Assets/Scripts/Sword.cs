...
public class Sword : MonoBehaviour
{
    ...
    public void GrabSword(ObjectAnchor sword, Projectile[] projectiles)
    {
        for (int i = 0; i < projectiles.Length; i++)
        {
            float threshold = projectiles[i].gameObject.transform.localScale.x;
            if (Vector3.Distance(gameObject.transform.position, projectiles[i].transform.position) < threshold + 0.3)
            {
